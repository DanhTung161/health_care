...
- Known limits: an Order with more than 250 Shopify transactions or a Product with more than 100 variants fails explicitly rather than silently truncating financial/catalog data. Catalog Product pages are capped at five because each Product includes nested connections. Article lookup by handle fails on cross-Blog ambiguity. Shopify HTML requires sanitization before later public rendering. Reconciliation has no automatic scheduler; retry failed Orders by rerunning the bounded range, and continue page-cap results with `nextCursor`.
