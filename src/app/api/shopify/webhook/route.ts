...
export async function POST(request: Request) {
  try {
    ...
    const shopDomain = normalizeShopifyWebhookDomain(request.headers.get('x-shopify-shop-domain'));
    if (!shopDomain || shopDomain !== config.domain) {
      return NextResponse.json({ success: false, error: 'Shopify shop domain mismatch' }, { status: 403 });
    }
    const deliveryId = shopifyWebhookDeliveryId(request.headers.get('x-shopify-webhook-id'));
    const topic = shopifyWebhookTopic(request.headers.get('x-shopify-topic'));
    let payload: unknown;
    ...
    if (error instanceof ShopifyError) {
      const status = error.code === 'NOT_CONFIGURED' || error.code === 'RATE_LIMIT' || error.code === 'SHOPIFY_API'
        ? 503
