...
## Phase 12: Shopify Integration Foundation

- Boundary: Shopify owns catalog, content, Orders, transactions, Refunds, and test/live state. Healthcare retains Patient, Appointment, clinical Charges/Billing, and the Phase 11 report. No Shopify Product/Variant/Customer MongoDB copies, Patient matching, cart, checkout, or storefront UI were added.
- Server-only Admin GraphQL client: `src/lib/shopify/client.ts`; configuration/error boundary: `src/lib/shopify/config.ts`. Admin reads require `SHOPIFY_SHOP_DOMAIN` and `SHOPIFY_ADMIN_ACCESS_TOKEN`; `SHOPIFY_API_VERSION` defaults to stable `2026-07`. Webhook verification additionally requires `SHOPIFY_WEBHOOK_SECRET`; its absence does not disable Admin reads. `SHOPIFY_STOREFRONT_ACCESS_TOKEN` is optional and unused in Phase 12. See `.env.example`; never use `NEXT_PUBLIC_` for secrets. Admin status is `NOT_CONFIGURED` only when Admin configuration is absent/invalid, `CONFIGURED` when valid, and `VERIFIED` only after `GET /api/shopify/status?verify=1` succeeds against the real API. The status response separately exposes `webhookConfigured`, never the secret itself.
- Minimum Admin scopes for implemented reads: `read_products` (Products, Variants, Collections), `read_content` (Blogs/Articles), and `read_orders` (Orders/transactions). Historical Orders beyond Shopify's default access window require `read_all_orders` approval. No write scopes. Configure only financial Order webhook topics: `orders/create`, `orders/updated`, `orders/paid`, `orders/cancelled`, `refunds/create`; no catalog/content subscriptions.
- Catalog functions in `src/lib/shopify/catalog.ts` return normalized DTOs from `types.ts` with bounded cursor pagination: Product by list/handle, Collections by list/handle, and Collection Products. Taxonomy category, merchant Product Type, Tags, and Collections stay separate. Nested variants are bounded; a truncated variant list fails explicitly. Media, options, pricing, and SEO fields come from Shopify. `content.ts` reads bounded Blog/Article pages and Article by handle. Admin Article/Blog 2026-07 lack the Storefront SEO field, so no fake SEO values are exposed. Later customer-facing rendering of Shopify HTML must sanitize it.
- Commerce projection: one `ShopifyOrder` per `(shopDomain, shopifyOrderId)` unique index, plus a transaction-date index for reporting. It stores no customer email, phone, or address. Successful SALE/CAPTURE count as gross cash; REFUND/CHANGE count as returned cash. Authorizations, voids, and suggested refunds are not collected cash. Order billed value is separate from cash. Shopify amount strings become safe integer minor units using ISO currency precision; VND has zero minor digits. Mongoose validates non-negative safe integers for order total, gross, refunded, and transaction amounts, and a signed safe integer for net cash. The synchronization service additionally rejects duplicate transaction IDs and derives totals from that normalized history; Mongoose update validators alone do not enforce cross-field conservation. No FX conversion.
- Public `POST /api/shopify/webhook` verifies raw-body HMAC-SHA256 with a timing-safe comparison, then validates shop domain, delivery ID, topic, and Order identity before fetching the authoritative Order from Admin GraphQL. The webhook body is not authoritative financial state. Financial idempotency comes from unique Order identity, authoritative transaction IDs, and conditional update by source `updatedAt`, not a delivery ledger. `lastWebhookId` is best-effort audit metadata for the last applied webhook; it is NOT a complete record of deliveries or retries. A repeated same-timestamp fetch is compared against a deterministic financial snapshot (transaction order-independent); identical state is unchanged, but differing state fails with `DATA_INTEGRITY` instead of silently skipping it. Shopify same-timestamp behavior still requires live verification; the correction deliberately does not invent a timestamp guarantee. `POST /api/shopify/reconcile` is ADMIN-only, bounded to 31 days, at most ten 50-Order pages, and returns created/updated/unchanged/error counts plus `complete` and a resumable `nextCursor` (`after` in the next request). No cron or Shopify mutation.
- Existing `GET /api/billing/reporting` keeps all Phase 11 fields and adds `data.sources.healthcare`, `.commerce`, and `.combined`. Commerce uses successful transaction occurrence dates, excludes test Orders by default, and groups by currency. Combined gross/refunded/net cash is available only when all live commerce in range is VND; otherwise it explicitly reports `MIXED_CURRENCY` and no combined sum. Net cash may be negative for a cross-period Refund. These are cash-flow metrics, not accounting-recognized revenue. Catalog/content reads never create Billing or Revenue.
- Verification without credentials: fixture tests cover missing configuration, HMAC, DTO normalization/pagination, minor-unit precision, Order financial normalization, retry/stale update behavior with mocked persistence, live/test filtering, cross-period Refunds, currency gate, and the unique index declaration. Local MongoDB was not reachable at `127.0.0.1:27017`; real concurrent index/CAS behavior and live Shopify API calls remain unverified. No real credentials were inspected or printed. The next operator should configure a Shopify app/scopes/webhook, set the server env vars, restart, call `GET /api/shopify/status?verify=1` as ADMIN, then invoke a bounded `POST /api/shopify/reconcile` with ISO `from`/`to` and review its errors/complete flag.
- Known limits: an Order with more than 250 Shopify transactions or a Product with more than 100 variants fails explicitly rather than silently truncating financial/catalog data. Catalog Product pages are capped at five because each Product includes nested connections. Article lookup accepts a Blog ID to disambiguate handles shared across Blogs and still fails safely when a handle-only lookup is ambiguous. Shopify HTML requires sanitization before later public rendering. Reconciliation has no automatic scheduler; retry failed Orders by rerunning the bounded range, and continue page-cap results with `nextCursor`.

### Phase 12B live verification (2026-09-21)

- The real Admin GraphQL connection, configured and reported API version `2026-07`, and configured shop-domain identity match were verified. Granted access included Product/Collection, content, recent Order, and historical Order reads.
- A bounded live Product, Product detail, Collection, Blog, Article, and Article detail read passed. Product normalization kept taxonomy category, merchant Product Type, Collections, Tags, Variants, and Options separate; the sampled Collection contained no Product.
- A real Article exposed a concrete handle-search issue: `hasNextPage` was not proof that the exact handle was duplicated. Detail lookup now accepts and validates the Blog GID from the list DTO, adds Shopify's `blog_id` filter, and retains fail-safe ambiguity handling for handle-only callers.
- The configured store returned no Order for both the default recent query and an explicit `status:any` query. Therefore real Order normalization, MongoDB persistence/unique-index enforcement, repeated/concurrent synchronization, reconciliation, and commerce-report integration remain blocked by missing live Order data; Phase 12B is not a full PASS.
- The webhook secret and a stable public HTTPS application URL were not configured. Existing financial-topic subscription inspection succeeded and found no subscriptions. Live webhook delivery was not observed.
- `.env.local` remained Git-ignored, no tracked Shopify credential literal was found, and the `ShopifyOrder` projection still contains no customer email, phone, or address fields.

## Phase 12C: Shopify Webhook Production Readiness

- Phase 12B status: **PASS WITH BLOCKED LIVE-ORDER GATES**. The real Admin API, identity, catalog, content, and access scopes were verified, but the configured store continued to return zero Orders. The operator lacks payment-gateway permission, so live Order normalization, Mongo projection, reconciliation, and commerce reporting were not misrepresented as complete.
- Required server environment: `SHOPIFY_SHOP_DOMAIN`, `SHOPIFY_ADMIN_ACCESS_TOKEN`, optional `SHOPIFY_API_VERSION` (default `2026-07`), and `SHOPIFY_WEBHOOK_SECRET` for webhook HMAC only. Admin reads remain available without the webhook secret. Never use a `NEXT_PUBLIC_` variable for an Admin token or webhook secret.
- Production delivery requires a stable public HTTPS base URL and `POST /api/shopify/webhook`. Subscribe only to `orders/create`, `orders/updated`, `orders/paid`, `orders/cancelled`, and `refunds/create`. Current deployment is blocked because no webhook secret or public HTTPS URL is configured; no subscription or tunnel was created and no live delivery was claimed.
- Processing order is fixed: read bounded raw bytes, verify raw-body HMAC-SHA256 with a fixed-length timing-safe comparison, validate configured shop/domain, delivery ID, and topic, then parse JSON and derive the Order ID. `refunds/create` uses Shopify's documented `order_id`. Invalid signature/shop/topic/metadata cannot trigger an Admin reload.
- Webhook financial fields are never authoritative. Every accepted topic reloads the Order through Admin GraphQL, applies the 250-transaction completeness boundary, normalizes successful SALE/CAPTURE and REFUND/CHANGE transactions, and conditionally updates the unique `(shopDomain, shopifyOrderId)` projection. `lastWebhookId` remains audit metadata, not financial exactly-once identity.
- Idempotency and ordering: duplicate deliveries and repeated authoritative reads are unchanged; an older `sourceUpdatedAt` cannot replace a newer projection; an identical same-timestamp financial snapshot is unchanged; a different same-timestamp snapshot fails `DATA_INTEGRITY`. Reconciliation remains the bounded recovery/backstop path.
- Admin GraphQL reads make at most three attempts. Transient network failures, HTTP 429, selected HTTP 5xx responses, and GraphQL `THROTTLED` errors use bounded exponential delay capped at two seconds and honor a safely capped `Retry-After`. Authentication, configuration, schema/query, invalid JSON, and data-integrity errors are not retried.
- Webhook HTTP policy: applied/unchanged/stale authoritative results return 200; invalid HMAC returns 401; wrong shop returns 403; malformed JSON, metadata, Order identity, or unsupported topic return 400; `DATA_INTEGRITY` returns 409; missing webhook configuration, exhausted rate limits, and exhausted transient Admin failures return 503; unexpected/Admin-auth/Mongo failures return 502. Non-2xx processing failures are deliberately not acknowledged as financially applied; Shopify retry is bounded and reconciliation remains the recovery mechanism.
- Local fixture verification covers all five financial topics, invalid/missing HMAC, wrong shop, unsupported topic, malformed JSON, missing/invalid delivery ID, duplicate delivery, out-of-order stale state, same-timestamp identical/conflicting state, authoritative reload failure, Mongo failure, and incomplete transaction history. Tests use synthetic IDs/data and persist no customer PII.
